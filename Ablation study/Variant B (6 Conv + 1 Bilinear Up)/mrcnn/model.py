...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    Returns: Masks [batch, num_rois, MASK_POOL_SIZE * 2, MASK_POOL_SIZE * 2, NUM_CLASSES]
    """
    # ROI Pooling
    ...
    x = KL.TimeDistributed(BatchNorm())(x)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(num_classes, (1, 1), activation="sigmoid"))(x)
    return x
...
class MaskRCNN():
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        self.history=self.keras_model.fit(
            ...
            use_multiprocessing=workers>1    #in mask cnn set true
        )
        self.epoch = max(self.epoch, epochs)
        try:
            l = self.history.epoch
            g = self.history.history['loss']
            z = self.history.history['val_loss']
            epoch_list.extend(l)
            tra_loss_list.extend(g)
            val_loss_list.extend(z)
        except Exception:
            pass
...
def call_back():
    l, g, z = return_value(epoch_list, tra_loss_list, val_loss_list)
    return l, g, z
